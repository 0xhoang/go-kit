...
func (s *ConfirmingAction) Task() {
	...
	if withdrawFsm.Current != taskCommon.CONFIRMING_STATE {
		s.TrackHistory(models.LOG_STATUS_FAILED, fmt.Sprintf("aass_state is not true, %v != %v, stage = %v", withdrawFsm.Current, taskCommon.CONFIRMING_STATE, s.model.AasmState), "")
	...
	s.TrackHistory(models.LOG_STATUS_SUCCESS, fmt.Sprintf("next step = %v", taskCommon.SUCCEED_EVENT), "")

	err := withdrawFsm.SendEvent(taskCommon.SUCCEED_EVENT, processingCtx)
	if err != nil {
		s.TrackHistory(models.LOG_STATUS_FAILED, fmt.Sprintf("SendEvent %v", taskCommon.SUCCEED_EVENT), err.Error())
		s.property.Dao.UpdateQueueRetry(s.property.Cfg.JobInfo.TimesRetry, s.model)
		return
	}
}
