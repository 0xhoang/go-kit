...
func (s *SubmittedAction) Task() {
	...
	if withdrawFsm.Current != taskCommon.SUBMITED_STATE {
		s.TrackHistory(models.LOG_STATUS_FAILED, fmt.Sprintf("aass_state is not true, %v != %v, stage = %v", withdrawFsm.Current, taskCommon.SUBMITED_STATE, s.model.AasmState), "")
	...
	s.TrackHistory(models.LOG_STATUS_SUCCESS, fmt.Sprintf("next step = %v", taskCommon.SUBMITED_EVENT), "")

	err := withdrawFsm.SendEvent(taskCommon.SUBMITED_EVENT, processingCtx)
	if err != nil {
		s.TrackHistory(models.LOG_STATUS_FAILED, fmt.Sprintf("SendEvent %v", taskCommon.SUBMITED_EVENT), err.Error())
		s.property.Dao.UpdateQueueRetry(s.property.Cfg.JobInfo.TimesRetry, s.model)
		return
	}
