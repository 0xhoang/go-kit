...
func (s *ProcessingAction) Task() {
	...
	if withdrawFsm.Current != taskCommon.PROCESSING_STATE {
		s.TrackHistory(common.LOG_STATUS_FAILED, fmt.Sprintf("aass_state is not true, %v != %v, stage = %v", withdrawFsm.Current, taskCommon.PROCESSING_STATE, s.model.AasmState), "")
	...
	s.TrackHistory(common.LOG_STATUS_SUCCESS, fmt.Sprintf("next step = %v", taskCommon.PROCESSING_EVENT), "")

	err := withdrawFsm.SendEvent(taskCommon.PROCESSING_EVENT, processingCtx)
	if err != nil {
		s.TrackHistory(common.LOG_STATUS_FAILED, fmt.Sprintf("SendEvent %v", taskCommon.PROCESSING_EVENT), err.Error())
		s.property.Dao.UpdateQueueRetry(s.property.Cfg.JobInfo.TimesRetry, s.model)
		return
	}
}
